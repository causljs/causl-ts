 ...
 *
 * Vitest configuration for the `@causl/devtools` package. Aliases the
 * `@causl/core` import specifier directly to the sibling package's
...
/**
 * Absolute path to `@causl/core`'s TypeScript entry, resolved relative to
 * this config file so the alias works regardless of the caller's CWD.
 */
const coreSrc = fileURLToPath(new URL('../core/src/index.ts', import.meta.url))

/**
 * Absolute path to `@causl/core`'s `/internal` entry — the adapter-level
 ...
 *
 * - `resolve.alias` redirects `@causl/core` to its in-repo source so test
...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // is matched before the `@causl/core` prefix is consumed.
      '@causl/core/internal': coreInternalSrc,
      '@causl/core': coreSrc,
    },
  },
  test: {
