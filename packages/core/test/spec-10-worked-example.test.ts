 ...
 * (the React binding ships the same shape under `@causlts/react`,
...
describe('SPEC §10 worked example', () => {
   ...
   * matches `@causlts/react`'s today (`(msg, graph) => graph`); to
  ...
  it('drives the same [4, 13, 301] sequence through the §8 MVU front door', () => {
    ...
    // shape `@causlts/react` ships and matches §8's listing.
    ...
    // `runMessages(update, graph, [...])` from `@causlts/react`.
