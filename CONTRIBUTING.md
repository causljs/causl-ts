...
`@causlts/core`'s wasm-pkg artefacts; `tools/engine-rs-core-bench/`
...
| 3 | Layering in §7 is enforced at the package boundary; `@causlts/core` does not export controller types | Open today — tracked by [#393](https://github.com/iasbuilt/causl/issues/393) (no mechanical gate yet; lint rule or fitness function pending) |
...
- **`@typescript-eslint/switch-exhaustiveness-check`** (configured in
  ...
  `@causlts/core/internal` — adapter packages import from there.
