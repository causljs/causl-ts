...
function detectReduxRules(ctx: ScanContext, node: ts.Node, emit: Emitter): void {
  if (ts.isCallExpression(node) && ts.isIdentifier(node.expression)) {
    ...
    if (isAliasOf(ctx.imports, 'react-redux', 'useDispatch', n)) {
      emit('R-03', node, 'useDispatch', 'Replace useDispatch with typed useDispatch<Msg>() from @causlts/react.')
      return
    }
    if (isAliasOf(ctx.imports, '@reduxjs/toolkit', 'createAsyncThunk', n)) {
      emit('R-04', node, 'createAsyncThunk', 'Replace createAsyncThunk with @causlts/sync resource(graph, key, loader).')
      return
    }
    if (
...
function detectCrossRules(ctx: ScanContext, node: ts.Node, emit: Emitter): void {
  ...
  if (ts.isSourceFile(node)) {
    ...
    for (const phantom of phantoms) {
      if (
        importedFrom(ctx.imports, '@causlts/react', phantom) ||
        importedFrom(ctx.imports, '@causlts/persistence', phantom) ||
        importedFrom(ctx.imports, '@causlts/core', phantom)
      ) {
