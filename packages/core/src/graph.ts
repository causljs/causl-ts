...
export function createCausl(options: CreateCauslOptions = {}): Graph {
  ...
  // `@causlts/core/wasm` entry point it lazy-loads on migration stays
  ...
  // the canonical `@causlts/react` adapter retains the `read()` return
  ...
  // the warning AND use `@causlts/react`'s canonical hooks: the
  ...
  // #1241 — adapter-exemption seam. Canonical `@causlts/react` hooks
  ...
  //
  // The seam is INTERNAL: it lives behind `@causlts/core/internal`'s
  ...
  function read<T>(node: Node<T>): T {
    ...
    //      adapter — e.g. `@causlts/react`'s `useCauslNode` — wraps
   ...
   * the counter sticky.
   *
   * Reachable only through `@causlts/core/internal`'s
   * `__causlAdapterRead(graph, fn)` helper. Canonical adapters
   * (`@causlts/react`'s `useCauslNode`, `useCausl`, `useCauslShallow`,
   ...
   *   through `@causlts/core/internal`'s `_migrateFrom(graph, snap)`
   *   helper. Adopters use {@link Graph.hydrate}; the migration
   *   path exists for the auto-adapt wrapper (`@causlts/core/wasm`'s
   ...
   *
   * @remarks
   * Reachable only via `dispose(graph, node)` from `@causlts/core/internal`;
  ...
  const backend: BackendEngine = new JsBackend({
    ...
    // `reduceResource` in `@causlts/sync/src/statechart-reducers.ts`)
  ...
  // via `@causlts/core/internal`'s `dispose(graph, node)`.
  //
  ...
  // `@causlts/core/internal`'s `dispose(graph, node)` can reach it without
