 ...
 * `@causlts/core` is the engine substrate and must not export
 ...
 * `packages/core/src/` whose content imports `@causlts/react`, and
...
describe('SPEC §17.3 — package-boundary layering enforcement', () => {
  it('flags `@causlts/react` imports from inside `@causlts/core`', async () => {
    ...
    expect(first.message).toMatch(/@causlts\/core/)
  })

  it('does not flag `@causlts/core/internal` from inside an adapter', async () => {
    // §12.3 seam — adapter packages may reach engine internals only
    // through `@causlts/core/internal`. Pin the positive case so a
    ...
    const source = `import { assertNever } from '@causlts/core/internal'\nexport const _ = assertNever\n`
  ...
  it('flags `@causlts/core/dist/...` deep imports from an adapter', async () => {
    ...
    const source = `import { x } from '@causlts/core/dist/graph.js'\nexport const _ = x\n`
