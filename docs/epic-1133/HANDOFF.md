...
- E.8 (#1451, PR #1452) — Bench harness wiring: 3 e2e roundtrip tests via `WasmBackend.subscribe(node, observer)` + `commit()` on the Phase-1 wrapper. Added `@causlts/core/wasm` alias to `packages/bench/vitest.config.ts`. Pure adopter-side integration.
