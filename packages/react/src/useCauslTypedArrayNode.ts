 ...
 *
 * Typed-array projection hook for `@causlts/react` (#688, sub-task of
 ...
 * {@link loadWasmBackend} from `@causlts/core/wasm`. The hook reads
 ...
 * `@causlts/core/wasm` subpath).
 *
 * @remarks
 * The dynamic `import('@causlts/core/wasm')` is deliberate: the
 * subpath is an opt-in entry point and should not be force-loaded by
 * `@causlts/react`'s main bundle. Bundlers that exclude the subpath
...
function probeWasmBackend(): Promise<boolean> {
  ...
  wasmBackendProbe = (async () => {
    try {
      ...
      const mod = (await import('@causlts/core/wasm')) as {
