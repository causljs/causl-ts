...
The `@causljs/sync-testing-internal` package sits at `packages/sync-testing-internal/` and ships only as a workspace dependency of `packages/sync/`. Its public surface is exactly the API sketch in §15.0: six generator factories (`resourceEventGen`, `conflictEventGen`, `conflictIdGen`, `loadingEpisodeGen`, `readScheduleGen`, `openSetSampleGen`), three harness factories (`propertyResource`, `propertyConflict`, `propertyConflictWithMap`), two event appliers (`applyEvents`, `applyConflictEvents`), and two shrinking-aware shape preservers (`preserveLoadingEpisode`, `preserveOpenPriming`). The package is `"private": true` and has the same `peerDependency` shape as `@causljs/core-testing-internal` (which we copied as the structural template, since the two packages do the same job for different surfaces — the engine for `@causljs/core`, the adapter for `@causljs/sync`). The eight property suites import everything they need from `@causljs/sync-testing-internal` and from `@causljs/core-testing-internal` (for `propertyTrials`); they import nothing else from inside `@causljs/sync`'s `src/` tree beyond the public re-exports a regular adopter would touch. That symmetry is deliberate: the property suite is an adopter from the perspective of the adapter's surface, and the harness factories are the seam that lets it stay one.
...
**The `@causljs/sync-testing-internal` package boundary is the second risk.** A workspace package is cheap to add but expensive to remove — once the eight property suites import from it, removing the package means rewriting eight test files. We asked whether the harness factories should live inside `@causljs/core-testing-internal` instead, since that package already ships `propertyTrials` and is the spiritual home of cross-adapter test infrastructure. We declined for two reasons. (1) The harness factories construct an adapter-side `Resource<T>` and `ConflictRegistry<T>` — both are types from `@causljs/sync`, not `@causljs/core`, and putting them in the engine's testing package would create a circular dependency edge `@causljs/core-testing-internal → @causljs/sync` that the engine's own test infrastructure refuses to depend on. (2) The §15.0 API sketch is explicit about the package name (`@causljs/sync-testing-internal`) and the path (`packages/sync-testing-internal/`); the spec is the source of truth for the surface, and the spec named a separate package because the engine-side and adapter-side test infrastructures have different consumers. We obey the spec.
...
import { resource, type ResourceHandle } from '@causljs/sync'
...
1. **`package_builds_with_tsc_noemit`.** A subprocess test that spawns `pnpm --filter @causljs/sync-testing-internal exec tsc --noEmit` and asserts exit code 0. Catches the failure mode where the package's `index.ts` re-exports a name that no source file actually defines.
2. **`public_api_matches_section_15_0_sketch_exactly`.** Imports `* as api from '@causljs/sync-testing-internal'` and asserts the union of `Object.keys(api)` is exactly the 13 names in the §15.0 API sketch (`resourceEventGen`, `conflictEventGen`, `conflictIdGen`, `loadingEpisodeGen`, `readScheduleGen`, `openSetSampleGen`, `propertyResource`, `propertyConflict`, `propertyConflictWithMap`, `applyEvents`, `applyConflictEvents`, `preserveLoadingEpisode`, `preserveOpenPriming`). Adding a 14th export, removing one of the 13, or renaming any of them is a red test before the property suite breaks.
3. **`package_is_workspace_only_and_excluded_from_publish`.** Reads `package.json` and asserts `private === true`. Asserts no `publishConfig.access === 'public'`. Asserts the package name does not appear in any other package's `dependencies` (it appears only in `devDependencies` of `@causljs/sync` and the adapter's property test files). The grep is over the workspace's `package.json` files.
4. **`importing_at_causl_sync_does_not_pull_in_testing_internal`.** Spawns a tiny ESM script that does `import * as sync from '@causljs/sync'` and asserts `Object.keys(require.cache)` (or its ESM equivalent — `import.meta.resolve` + a Node `--experimental-vm-modules` introspection) does NOT contain a path under `packages/sync-testing-internal/`. The test is the structural witness that the production bundle of `@causljs/sync` cannot accidentally pull in the test infrastructure. The same shape is used today for `@causljs/core-testing-internal`; we copy the assertion.
...
1. **Workspace-only.** The package is `"private": true`; `pnpm publish --filter @causljs/sync-testing-internal` exits with a clean refusal rather than uploading the package. The same `private: true` flag protects the engine's `@causljs/core-testing-internal` today; we lift the pattern unchanged.
2. **API closure.** Every export from `@causljs/sync-testing-internal` is named in §15.0's API sketch. A 14th export is a §15.0 amendment, not a quiet addition. Test 2 above is the runtime gate; the §15.0 prose is the spec gate.
3. **Bundle isolation.** `@causljs/sync`'s production bundle does not include `@causljs/sync-testing-internal`. The `peerDependencies` declaration on the testing-internal package (which ties it to `@causljs/sync` at workspace-link time) does NOT pull the testing-internal package's source into `@causljs/sync`'s production bundle, because `@causljs/sync` does not declare a `dependency` on the testing-internal package — only the eight property test files do, via the workspace-level `devDependency` we add to `packages/sync/package.json`. Test 4 is the structural witness.
...
The §15.0 Property 4 predicate. The generator is `readScheduleGen()` from `@causljs/sync-testing-internal`, which produces samples of shape `{ readsBeforeResolve: number, secondEpisodeReadsBeforeResolve: number }` with bounded counts. The predicate body opens a loading episode, reads the resource state `readsBeforeResolve` times and stashes the `promise` reference from each read; settles the episode; opens a second loading episode; reads the state `secondEpisodeReadsBeforeResolve` times and stashes those promise references; then asserts pointer equality within episode A, pointer equality within episode B, and pointer inequality between A and B (provided both episodes had at least one read).
...
3. The four-arm DU is closed by construction in `@causljs/sync`'s source; the property is the runtime witness against fuzzer-discovered escape.
...
3. The `@causljs/sync-testing-internal` package was loaded once per property suite — the harness factories are not duplicated across the eight files.
...
2. **The `@causljs/sync-testing-internal` package builds clean.** `pnpm --filter @causljs/sync-testing-internal exec tsc --noEmit` exits 0; `pnpm --filter @causljs/sync test` discovers and runs all eight property files.
...
6. **The eight property suites are seed-deterministic.** `CAUSL_FUZZ_SEED=42 pnpm --filter @causljs/sync test` produces the same trial sequence on two consecutive runs; a counterexample on one workstation reproduces on every workstation.
...
8. **The package is bundle-isolated.** TASK 9.1 test 4 passes; importing from `@causljs/sync` does not transitively pull in `@causljs/sync-testing-internal`; the production bundle of `@causljs/sync` is unchanged in size and shape.
...
- Bundle-budget sub-imports. The `@causljs/sync-testing-internal` package is `private: true` and excluded from the production bundle by construction (TASK 9.1 concern 3); we do not extend the bundle-budget tooling to cover sub-imports of an unpublished package.
...
| `@causljs/sync-testing-internal` package | §15.0 API sketch |
