 ...
 * packages (`@causlts/react`, `@causlts/persistence`,
 * `@causlts/devtools-bridge`, …) that need engine-internal hooks the
 * application surface deliberately omits.
 *
 * @remarks
 * Application code MUST NOT import from `@causlts/core/internal`. The
 ...
 * and NOT covered by SemVer guarantees on the `@causlts/core` public
 ...
 * disposal hook is owned by `@causlts/react`'s `useCauslFamily`
...
/**
 * Sentinel marker confirming the `@causlts/core/internal` entrypoint
 ...
 * @internal
 */
export const INTERNAL_ENTRYPOINT = '@causlts/core/internal' as const;
 ...
 *
 *   1. The WASM auto-adapt wrapper (`@causlts/core/wasm`'s
 ...
 * inside the body.
 *
 * Used by canonical `@causlts/react` hooks (`useCauslNode`,
 ...
 * boundary. Adapters (`@causlts/react`, `@causlts/persistence`,
 * `@causlts/devtools-bridge`) wrap their selector / listener boundary
 ...
 * leaks.
 *
 * Adapter packages (`@causlts/react`, `@causlts/persistence`,
 * `@causlts/devtools-bridge`) wrap their selector/listener boundary
