...
- `narrowCapability(graph)` is now exported from `@causlts/core/internal` (capability narrowing is also relevant to adapters, not just tests; see #372 / #376 / #385).
- `renderSpy<P>(Component)` was scoped out of this engine-level seam — render-counting belongs in the consuming framework's own test helpers, not in `@causlts/core/testing`. `@causlts/react` callsites use `act`/`render` from `@testing-library/react` directly.

## Anti-features (deliberately not here)

- **No production code.** This module is excluded from `@causlts/core`'s published bundle (`package.json` `exports` field exclusion + `size-limit` lint).
...
The testing surface is versioned independently of `@causlts/core`'s public API. A change here is **not** a public API change of the engine. The contract for users of this module: imports look like `import { recomputeCounter } from '@causlts/core/testing'`.
