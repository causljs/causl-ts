/**
 * @causlts/devtools-bridge — `connectDevtools`.
 *
...
import type { Graph, GraphSnapshot, GraphTime, Unsubscribe } from '@causlts/core'
 ...
 * `narrowCapability` from `@causlts/core/internal` is *not* applied
...
function buildHandlers(ctx: HandlerContext): Handlers {
  const table: Handlers = {
     ...
     * adapters (e.g. `@causlts/persistence`) read `ctx.state.persist`.
     */
    TOGGLE_PERSIST: () => {
