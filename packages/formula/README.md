...
> [@causlts/core](../core/). The core engine knows nothing about `=A1+B1`;
> this package translates AST → derived node.

## Install

```bash
pnpm add @causlts/formula @causlts/core
```
...
the same wiring this README describes against `@causlts/core` and
`@causlts/devtools`. From the repo root:

```bash
pnpm --filter @causlts/formula run demo
```
...
1. **`parseFormula` consumes IR types directly.** `parseFormula` and
   ...
   internal IR module instead of `./grammar.js`. The `@causlts/formula`
   ...
   `@causlts/formula/grammar` will see a moved-source identity.
