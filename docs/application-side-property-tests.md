...
Wrap the call to `fc.assert` with `propertyTrials('your-label')` from `@causlts/core/testing` so the §15.2 1000-trial floor is enforced and the seed logs a reproducible `CAUSL_FUZZ_SEED=…` hint on failure.
...
import { propertyTrials } from '@causlts/core/testing'
