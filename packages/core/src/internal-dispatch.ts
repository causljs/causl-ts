 ...
 *
 * Private-to-`@causlts/core` registry that bridges the engine closure
 * (in `graph.ts`) and the publicly-importable `@causlts/core/internal`
 ...
 * surface behind `@causlts/core/internal`, this module is excluded
 ...
 * hook below, for instance, is owned by `@causlts/react`'s
...
export interface InternalDispatch {
   ...
   * `@causlts/core/internal`'s `_migrateFrom(graph, snap)` helper;
   ...
   * `@causlts/react` hooks (`useCauslNode`, `useCausl`,
   ...
   * `@causlts/core/internal`'s `__causlAdapterRead(graph, fn)`
  ...
  if (!d) {
    throw new Error(
      'Graph was not produced by createCausl() — internal dispatch unavailable. ' +
        'Did you pass an unrelated object to an @causlts/core/internal helper?',
    );
  }
  return d;
}
