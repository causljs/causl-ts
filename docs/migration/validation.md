...
> (`@causlts/core` 6 KB, `createCausl`-only 4 KB, `@causlts/react` 3 KB,
> `@causlts/devtools-bridge` 3 KB). PR #458 retired those ceilings —
...
> they ratchet `@causlts/core (full import)` at 20 KB,
> `createCausl`-only at 15 KB, `@causlts/react` at 8 KB,
> `@causlts/devtools-bridge (connectDevtools-only)` at 5 KB, and
> `@causlts/sync` at 12 KB. The wasm-pkg cells (serde-json /
...
to production. `@causlts/migration-check` itself is a build-time
...
npx @causlts/migration-check > drift-before.json
...
npx @causlts/migration-check > drift-after.json
