...
Three minimal fixture apps that import `@causlts/core` (main barrel) and
dynamically import `@causlts/core/wasm` (lazy-load entry point per #684).
...
1. **`@causlts/core` main barrel bundles cleanly** — the package exports
   only TypeScript / JS; no native or WASM deps reachable.
2. **`@causlts/core/wasm` lazy import produces a separate chunk** — the
   loader from #684 (`packages/core/wasm/index.ts`) is reachable via a
   `import('@causlts/core/wasm')` call without bundle-time errors. Phase-1
...
mirror how an external adopter consumes `@causlts/core`. Pulling them
...
Each fixture references the workspace `@causlts/core` via a `file:`
specifier in its `package.json`, so the CI step
`pnpm --filter @causlts/core build` runs first and the bundler picks up
the freshly-built `dist/` directly.

## Run locally

```sh
pnpm --filter @causlts/core build
