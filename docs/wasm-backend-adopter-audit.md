...
everything `createCausl(...)` returns, the `@causlts/react` adapter API,
and the documented adopter hook at `@causlts/core/internal` — and lists,
...
- **Adopter-visible?** — `Y` if the symbol is part of the documented
  ...
  only via the deep-import contract `@causlts/core/internal`. The N
...
| `NodeDisposedError`                                                                                             | class             | error       | Thrown when accessing a disposed node.                                                                                                                                   | Tombstone map lives WASM-side; `dispose` (the `@causlts/core/internal` adopter hook) writes the tombstone.                                                                                                                                                                                                                                                                                                                                                                             | Y                |
...
| `useCauslFamily(factory)` / `FamilyFactory` / `FamilyGraph`                                                     | hook / types      | react       | Per-key node identity within a provider; refcount-driven `dispose` via `@causlts/core/internal`.                                                                           | Disposal hits WASM-side (see `dispose` row). The refcount lives JS-side.                                                                                                                                                                                                                                                                                                                                                                                                             | Y                |
...
| `assertNever` (re-exported)                                                                                     | function          | (utility)   | Exhaustiveness probe surfaced from `@causlts/react`.                                                                                                                       | (no boundary).                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Y                |
| `VERSION` (`@causlts/react`)                                                                                      | value             | construct   | Pinned `'0.0.0'`.                                                                                                                                                        | (no boundary).                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Y                |
...
- Internal (N) rows: 8 — three core internals (`DisposalDuringCommitError`,
  ...
  `@causlts/core/internal` deep-import hooks.
...
interface BackendEngine {
  // Construct / lifecycle
  dispose(node: NodeId): void // from @causlts/core/internal
...
- **`@causlts/core/internal` capability narrowing
