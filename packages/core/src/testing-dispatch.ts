 ...
 * production `@causlts/core/internal` entrypoint exposes adapter
 * primitives, while this registry is consumed only by the
 * `@causl/core-testing-internal` package surfaced as
 * `@causlts/core/testing`. Splitting the two registries keeps §12.3
  ...
  if (!d) {
    throw new Error(
      'Graph was not produced by createCausl() — testing dispatch unavailable. ' +
        'Did you pass an unrelated object to an @causlts/core/testing helper?',
    );
  }
  return d;
}
