...
- **Property fuzz** at the 1000-trial floor (per `spec-15.2-conformance.test.ts` walker enforcement) for `@causlts/core`'s closed-DU invariants.
...
2. **Adopter request.** A team integrating `@causlts/sync` or `@causlts/devtools-bridge` requests deeper bounded-enumerator coverage on a specific PR by applying the `race-detection-tier-2` label.
