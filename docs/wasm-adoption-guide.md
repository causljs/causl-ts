...
import { useCausl, useCauslCommit } from '@causlts/react'
...
```ts
import { useMemo } from 'react'
import { useCausl, useEngineTelemetry } from '@causlts/react'
...
`@causlts/core/wasm` ships with a default loader that resolves the
...
version string (`VERSION` exported from `@causlts/core`). A
deployment-time script that copies `node_modules/@causlts/core/wasm-pkg/`
into your asset server should preserve the version segment so the
SRI hashes in the preload links stay correct:

```sh
# deploy-time
VERSION=$(node -e "console.log(require('@causlts/core').VERSION)")
cp -r node_modules/@causlts/core/wasm-pkg ./public/causl/$VERSION/wasm-pkg
```

And at runtime:

```ts
import { VERSION } from '@causlts/core'
import { loadWasmBackend } from '@causlts/core/wasm'
...
The `backend: 'js'` short-circuit never imports `@causlts/core/wasm`
...
(`@causlts/sync`, embedded use-cases) opt in per graph without
