...
5. **I will ship the smallest thing that earns the right to grow.** The public Graph surface is a single-digit canonical set in §12.1 plus a curated second tier in §12.2 — **twenty-one public Graph members today (six canonical members plus fifteen second-tier; the seventh canonical entrypoint, `createCausl`, is the construction factory, not a `Graph` member), audited quarterly.** Rust core, GraphQL, Postgres, WASM are not phases of this spec; they live in §13 with explicit reopen triggers or they do not appear at all.
...
- A spreadsheet engine. I will support spreadsheet *patterns* on top of the core through `@causlts/formula`; I will not ship VLOOKUP.
- A database. There is no query language, no index, no on-disk storage format. The commit log is a bounded ring buffer in process memory; persistence is an adapter (`@causlts/persistence`) that snapshots inputs at chosen commits.
...
- A virtual DOM, a renderer, or any kind of view layer. `@causlts/react` translates engine commits into `useSyncExternalStore` snapshot semantics; the rendering decision is React's, not the engine's.
...
§5.1 Amendment 5 (review 2026-07-06) — Phase-G/H observable environment and observer-error policy. Two facts the engine enforces, previously unpinned by §5.1:
- **Observable environment.** A Phase G (per-node) or Phase H (commit-level) observer frame observes **post-F.6** state: `now` is the *new* GraphTime, `graph.commitLog` already includes the current commit, and `readAt(·, now)` resolves. This ordering is normative: a native/FFI backend that fires observers from inside its commit primitive (§5.1 Amendment 4) MUST present the same post-F.6 environment to observer frames, so an in-frame `readAt(derived, now)` does not diverge from the value-of-record engine.
- **Observer-error default.** A throwing observer is **caught**, reported via the `onObserverError` hook (`createCausl({ onObserverError })`; the §6.2 ObserverError sub-region §5.3 names its `ObserverErrorContext.source` labels), and dispatch **continues** with the remaining observers. The default policy when no hook is supplied is **report-and-continue** (`defaultOnObserverError`): a subscriber throw does **not** prevent the remaining observers from firing and does **not** fail the commit — `commit()` does not throw on an observer error. This continue-on-error semantics is required for the cross-backend fire-order determinism contract §5.1 Amendment 4 names: a backend that aborted the fan on the first throw would produce a different `changedNodes` observation stream and diverge.
...
The commit window is a load-bearing invariant the engine enforces, and it is stated here so a conformant re-implementation cannot diverge while satisfying every other clause.

**The window.** The engine is in state `Committing` from the moment Phase A is entered until Phase H completes. The window therefore *includes* the Phase G/H observer-dispatch frames and the Phase D compute bodies — it does not close when Phase E freezes the `Commit`, nor when the catch arm's rollback envelope (Phases B–F.6, §5.2) ends. This is deliberate: a native/FFI backend that runs its Phase-G/H observer fan synchronously inside its commit primitive (the shape §5.1 Amendment 4 anticipates) must keep the window open while observers fire, so the invariant is pinned at the phase level rather than left to the substrate.
...
Why this needs to be written down: the §5.2 phrasing (Phases G/H "outside the rollback envelope") could be read as "the commit has ended when observers fire," inviting a conforming implementation to clear its committing flag before Phase G and accept commit-from-subscriber. That reading diverges from the shipped engine while satisfying every other written clause; §5.7 closes the gap.
...
**Controller region** (conceptual, not implemented). The diagram preserves the region as `CONCEPTUAL` so readers see the model / controller / engine separation §7 names, drawn at the same fidelity as the regions that ship. The states the diagram sketches — `Idle | Selecting | Drawing | Inspecting` — are placeholders; no engine code reads them. The reopen trigger is the same one §13 names for any deferred capability: when a devtools-bridge or application-layer use case actually needs a controller-state region tagged in the engine, it lands here, with the regions rebalanced and the chart updated. Until then, the controller layer is application-supplied (§7.2), and the conceptual region is the teaching diagram that explains *why* it lives outside `@causlts/core`.
...
The package layout enforces the layering: `@causlts/core` is the engine substrate; `@causlts/react`, `@causlts/sync`, `@causlts/persistence`, `@causlts/devtools`, `@causlts/devtools-bridge`, `@causlts/formula` are adapters that bridge the substrate to specific controller / view technologies; the application owns the model layer and supplies the controller layer.
...
`ReadOnlyGraph = Pick<Graph, 'read' | 'subscribe' | 'subscribeCommits' | 'now'>` is the canonical narrow type — the read-side capability slice every adapter needs and nothing more. It is `@causlts/core/internal`-only per §12.3: applications should not name `ReadOnlyGraph` in their own types because the adapter that supplies the narrowed handle is the one that owns the slice. Per #371, #433, and #441, each adapter declares its own `Pick<Graph, ...>` alias for the slice it actually needs:

- `@causlts/react`'s selector boundary uses `Pick<Graph, 'read' | 'subscribe' | 'now'>` (no `subscribeCommits` — selectors don't need the commit firehose).
- `@causlts/persistence`'s observer uses `Pick<Graph, 'read' | 'subscribeCommits' | 'now' | 'commitLog'>` (no per-node `subscribe` — persistence reacts to commits, not to individual node changes).
- `@causlts/devtools-bridge`'s host channel uses `Pick<Graph, 'read' | 'subscribeCommits' | 'now' | 'commitLog' | 'snapshotAt' | 'readAt' | 'dependencies' | 'dependents'>` (no `commit`, no `hydrate` — the bridge is read-only by §13's view-only-JUMP discipline).
...
- **Inversion gate** — `@causlts/core` may not import from any sibling adapter (`@causlts/react`, `@causlts/sync`, `@causlts/persistence`, `@causlts/devtools`, `@causlts/devtools-bridge`, `@causlts/formula`, `@causl/checker`, `@causl/bench`, `@causlts/migration-check`). The engine substrate is the bottom of the layer stack; reaching into an adapter inverts the layering and creates the kind of two-way coupling that lets controller-shaped types leak back into the core barrel. The error message names the spec section (`SPEC §17.3 / §7`) so a developer who hits the rule has the rationale one click away.
- **Deep-path gate** — adapter packages may reach engine internals only through the documented `@causlts/core/internal` entrypoint, never through `@causlts/core/dist/*` or `@causlts/core/src/*`. The `exports` field in `packages/core/package.json` already refuses to resolve such paths at runtime; this rule codifies the same contract at lint time so a future bundler or tsconfig change cannot silently re-open the back door.
...
3. **IR-level static rule in `causl-check`.** The Rust binary already runs as a CI gate (§16); a layering pass would walk `CauslModel.nodes` and assert no node's compute reaches across a layer boundary. Rejected on scope: the IR is the contract between `@causlts/core` and the checker, and layering is a contract between `@causlts/core` and the *adapter packages* — a different boundary at a different abstraction level. Layering violations live in import graphs, not in the IR. The lint rule sits at the right boundary.
...
The team rejected a "controller layer in `@causlts/core`" alternative (a controller-shaped type that ships with the engine, so applications inherit a controller vocabulary) because the controller is the most application-specific layer in the §7 stack — a spreadsheet's controller, an asset-hierarchy editor's controller, and a booking system's controller share almost no vocabulary. Shipping a generic controller in `@causlts/core` is the same mistake the previous draft made by listing eleven inspirations and stacking them with plus signs. The controller layer is application-supplied; the conceptual region in §6 is the teaching diagram that explains why.
...
```ts
import { defineMsgs, payload, type MsgOf, assertNever } from '@causlts/react'
...
```ts
import { defineMsgs, payload, type MsgOf, createUpdate } from '@causlts/react'
...
- **`whyUpdated(node)`** (in `@causlts/devtools`) — `DerivedNode<WhyUpdatedResult>`. Returns a derived node that recomputes once per commit and answers "what most recently caused this node to update?" Tags: `recomputed | directly-set | no-cause`.

- **`whyNotUpdated(node)`** (in `@causlts/devtools`) — `DerivedNode<WhyNotUpdatedResult>`. The dual of `whyUpdated`. Tags: `did-update | no-dep-overlap | object-is-deduped | no-cause`.
...
| `Graph.stats` | `() => EngineTelemetry` | Engine telemetry snapshot: live input/derived counts, node version, and a state hash, returned as the frozen `EngineTelemetry` record. |
...
### 12.3 Internal-only adapter entrypoint (`@causlts/core/internal`)

A separate entrypoint with strict capability discipline. Adapter packages import these via `@causlts/core/internal`; application code never does. An ESLint rule (#393, #435) enforces the layering at the import statement.

| Surface | Used by | Why internal |
| --- | --- | --- |
| `dispose(graph, node)` | `@causlts/react` `useCauslFamily` lifecycle (#124) | Adapter-owned lifecycle. |
| `narrowCapability(graph)` | `@causlts/react` selectors and listeners | Mark Miller's principle of least authority. |
...
#### 12.3.1 Testing seam (`@causlts/core/testing`, npm: `@causl/core-testing-internal`)
...
Three entrypoints, three audiences: `@causlts/core` for application code, `@causlts/core/internal` for adapter code, `@causlts/core/testing` for test code. The boundaries are mechanically enforceable.
...
`@causlts/core` is the most-watched contract; adapter packages change at their own pace. SPEC §12 audits only the `@causlts/core` `Graph` interface. Each adapter's public surface lives in its own README:

| Adapter | README |
| --- | --- |
| `@causlts/react` | `packages/react/README.md` |
| `@causlts/sync` | `packages/sync/README.md` |
| `@causlts/persistence` | `packages/persistence/README.md` |
| `@causlts/devtools` | `packages/devtools/README.md` |
| `@causlts/devtools-bridge` (private; §13 deferred-product) | `packages/devtools-bridge/README.md` |
| `@causlts/formula` | `packages/formula/README.md` |
| `@causl/checker` | `packages/checker/README.md` |
| `@causlts/migration-check` | `packages/migration-check/README.md` |
...
- **JUMP path in `@causlts/devtools-bridge`** — switches from view-only `snapshotAt(t)` to branch-forking when `parentTime` is set.
...
| Multi-tab app with BroadcastChannel | One `createCausl()` per tab; `@causlts/sync` brokers cross-tab |
...
`@causlts/formula` ships as a separate adapter package; the core itself does not parse expressions, does not know about `=SUM(...)`, does not know about cell references. Reopen trigger for folding into core: none.

### 13.6 Async resources — DEFERRED (via separate adapter)

`@causlts/sync` ships as the adapter that models async fetches as Events feeding Inputs, with the lifecycle in §6. Reopen trigger for folding into core: none.
...
Out of scope. If an adopter ships GraphQL on top of `@causlts/core` they ship it as their own adapter.

Reopen trigger: an adopter shipping GraphQL on top of `@causlts/core` whose pattern is broadly enough useful to fold into a maintained sibling adapter. None today.

### 13.10 Devtools as a shippable product — NOT-PLANNED

I'll ship inspection primitives (§11) — `whyUpdated`, `whyNotUpdated`, `commitLog`, `dependencies`, `dependents`, `liveDerived`, `replaceMany`, `commitMetadataDerived`. **I will not ship a devtools UI as a product.** The `@causlts/devtools-bridge` package is the wire-protocol substrate that a downstream UI *would* consume; it ships as private precisely so a future UI initiative can ship it as its product.
...
| --- | --- | --- |
| Engine: `O(|affected|)` recompute | `perf-invariant — SPEC §14 gate` | `pnpm --filter @causlts/core run test:perf-invariant` runs `packages/core/test/perf-recompute.test.ts` (#145). |
| React: re-render scope | `perf-invariant — SPEC §14 React subscription gate` | `pnpm --filter @causlts/react run test:perf-invariant` runs the `useCausl`, `useCauslShallow`, `useSyncExternalStore`, `strictMode`, and `family-grid` suites; the family-grid leg ratchets the heap-delta gate under `CAUSL_HEAP_GATE=1` (#389). |
...
| --- | --- | --- | --- | --- | --- |
| 1 | **Source-mapped error chains across the eleven typed-error hierarchy** — every `CycleError`, `NodeDisposedError`, `HydrationSchemaError`, `CapabilityViolation`, etc. carries `{ commit: Commit, site: CallSite, dependents: NodeId[] }` via `Error.cause` | Every error in `@causlts/core` is a graph-shape error, and graph-shape errors are unsolvable without graph context. The current class-name-plus-string story is a lie about typed errors. | ~3 KB | ~4 KB | Hejlsberg / Eich |
| 2 | **Devtools wiring as a one-line opt-in (`createCausl({ devtools: true })`)** — `connectDevtools` from `@causlts/devtools-bridge` folded into core, dead-code-eliminated when the flag is `false` or `NODE_ENV === 'production'` (extension-absent path) | Adopters install the bridge package on day six, after the first incident. Hiding devtools behind an extra install is self-sabotage; React DevTools succeeded by being the default. | ~0 KB (DCE) | ~3 KB | Markbåge / Linsley |
...
| --- | --- | --- |
| `@causlts/core (full import)` | **20 KB** | Production minified. The 15-30 KB band's working target plus ~2 KB headroom for the next adopter-driven addition. |
| `@causlts/core (createCausl-only)` | **15 KB** | The minimal import path — `createCausl`, the canonical seven, and the §14.2.1 additions that ride along. Imports of §12.2 second-tier methods grow the cell-size; consumers who only register inputs and read derivations stay near this floor. |
| `@causlts/react` | **8 KB** | Increased from the previous 3 KB to absorb the §8.2 extension hooks (`useCauslShallow`, `useCauslFamily`, `useCauslSuspense`, `<Hydrate>`) at honest size, plus the dev-mode "Update did not commit" warnings (Dan Abramov championed) DCE'd in production. |
| `@causlts/devtools-bridge (connectDevtools-only, absent-extension path)` | **5 KB** | Increased from 3 KB to absorb the bridge logic that now opts into core via `devtools: true`. The absent-extension path remains the floor; the present-extension path adds real wiring above this number, gated by `__CAUSL_DEVTOOLS__`. |

Each ceiling is a CI gate (the existing `size — bundle-size gate (#147)` job). A PR that crosses any ceiling fails the gate; the PR description must include the §14.2.1 written team consensus or the size-limit bump is rejected. The ceilings are budget caps, not targets — the team works to the **18 KB target on `@causlts/core (full import)`**, with the 20 KB ceiling reserved for the cycle's next addition.
...
- **`useTransition`-aware MVU runner.** Sebastian Markbåge wants this in core; Evan Czaplicki pushes back hard ("framework-agnostic core is incompatible with React-scheduler-aware code"). Unresolved. Likely outcome: stays in `@causlts/react` until a non-React adapter forces the question.
- **Edit-mode time travel via `Commit.parentTime`.** Alan Kay wants the design in core (§13.2 already engineers it); Trygve Reenskaug vetoes core involvement ("a tool concern; ship it in `@causlts/devtools` where roles like Historian and Editor live"). David Harel splits the difference: design lands in core (the `parentTime` field), UX lands in devtools. The split is honest but does not yet pay for itself.
- **`liveDerived` and `replaceMany` promoted from `@causlts/devtools` to core.** Kay yes (the §11 REPL primitives), Reenskaug hard no (role primitives), Harel against on chart-edge grounds, Conal Elliott neutral. Three-to-one against; stays in `@causlts/devtools`.
- **Bundled persistence with localStorage as default.** Tanner Linsley wants it in core; Matt Pocock pushes back on the storage-adapter pluggability argument. Compromise on the table: ship the `StorageAdapter` *protocol* in core and keep concrete adapters (`localStorage`, IndexedDB) in `@causlts/persistence`. Not yet ratified.
...
- **`useCauslQuery(fetcher)`, `commitMutationQueue` in core.** Server-state and offline-first are real patterns, but neither is universal. Adopters reaching for them are already reaching for Query / SWR; relitigating that fight inside core is wrong. Stay in `@causlts/sync`.
- **`@causlts/formula`, `@causl/checker`, `@causlts/migration-check` folded into core.** Tooling-adjacent, separate release cadence. Migration-check especially — a one-time-per-upgrade tool, not a runtime dependency.
...
1. **Multi-process and multi-tab races are out of scope.** The property suite runs in a single process. `@causlts/sync` handles the multi-process case at the network seam; the *cross-graph* race class is the §13 "multi-user synchronisation" item.
...
What ships in v1 is a **static IR linter** — `@causl/checker` (npm wrapper) plus the Rust binary `causl-check`, running a fixed set of one-shot passes over a JSON IR exported from `@causlts/core`. What this section originally drafted — a bounded model checker that exhaustively enumerates async-resolution orderings, message-dispatch orderings, and conditional-derived branches — is **deferred and not planned** (#272 closed not-planned; EPIC #282 parks the work).
...
`CauslModel` is the wire format between `@causlts/core` and `causl-check`. Its shape is closed at seven top-level fields and at two `kind` constants (the two-`kind` `IRNode` discipline preserved across the schema 2 → 3 widening that EPIC-1 PR-A applied; see §16.2.1 for the full schema 3 specification):
...
export interface IRDerived {
  ...
  /**
   * RESERVED-EMPTY (review 2026-07-06, M7). The shipped exporter
   * emits `[]` unconditionally — there is no populator today. The
...
export interface IRSubscribe {
  ...
  readonly graphId: IRGraphId
  /**
   * Subscription id. NOTE (review 2026-07-06, L5): the shipped
   * exporter mints this id **per export**, not at registration, so
...
- **`@causlts/devtools-bridge`** — schema-3 PR adds projection of `events`; until then, ignores the field (forward-compatible).
...
The wire format is intentionally JSON, not bincode: `@causlts/core` is TypeScript, the worker is Node, and JSON is the protocol every Node program already speaks. The `dashmap` content-addressed cache makes a hot-path hit O(1) without crossing the process boundary, paying back the parse overhead.
...
```ts
import { hypothesis, always, afterCommit, implies } from '@causlts/hypothesis'
...
2. **Async resolution + Msg trajectory.** State space extends to pending resource resolutions and message-dispatch traversal. Acceptance: rows 2, 6 on a fixture with `@causlts/sync` resources.
...
| 6 | Stale-async resolution | RUNTIME-ONLY | `originGraphTime` compare in `@causlts/sync`. Network is the oracle. |
...
| 4 | Every discriminated union in §9 is a tagged union with exhaustiveness via `assertNever`. | MECHANICAL | `assertNever` from `@causlts/core/internal`; per-adapter `*.test-d.ts` exhaustiveness fixtures. |
...
| 14 | The opt-in `@causlts/core/wasm` substrate ships against a documented **three-tier host compatibility matrix** (`wasmgc-builtins`, `wasmgc-classic`, `serde-json`) with a fall-through fallback to the TS engine; no adopter is ever stranded on an unsupported host because every supported host either runs at least one tier or hits the documented JS-engine fallback path per §17.6 (#690 amendment). | DESIGN-DISCIPLINE | `packages/core/wasm/README.md` host-tier table + `detectBridge()` fallback chain; `docs/wasm-adoption-guide.md` adopter checklist; §17.6 host-tier table and the four-tier feature-detection contract. |
...
| --- | --- | --- | --- |
| `@causlts/core` main bundle | unchanged from pre-EPIC | 20 KB (§14.2) | Zero WASM imports, zero loader code — adopters who never call `loadWasmBackend()` pay nothing |
| `@causlts/core/wasm` entry stub | 630 B Brotli | 3 KB ceiling (per #684 size-limit) | Public API surface: `loadWasmBackend`, `detectBridge`, `loadStreaming`, `wasmUrlFor`, `WasmBackendUnavailableError`, `BridgeFeatures` |
...
import { loadWasmBackend, WasmBackendUnavailableError } from '@causlts/core/wasm'
...
**Contract sentence:** the §17.6 commitment-14 row's 200 KB raw / 80 KB Brotli ceiling for the Tier 3 (`wasm-pkg/serde/`) artefact is **not** relaxed. The current-state callout immediately under the table records: the v0.9.0 artefact ships at **213 KB raw / 66 KB Brotli** (Brotli 14 KB under the canonical 80 KB cap; raw 13 KB over the canonical 200 KB cap); the root `package.json` size-limit cell `@causlts/core wasm bridge — serde-json (raw)` is set to **230 KB** (ratcheted by PR #1112) so the bundle-size CI gate absorbs the measurement without false-firing on the documented gap; the Brotli post-build ceiling for the serde bridge is documented at **70 KB** in `packages/core/wasm-pkg/README.md` (enforced by `pnpm wasm:build`, not by a size-limit cell — the `//size-limit-wasm` comment in root `package.json` names that boundary). The divergence is recorded per Option C of #1150 (document the gap rather than silently exceed the SPEC).
