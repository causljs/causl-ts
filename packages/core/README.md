...
**Build & distribution:** this is the **causl-ts** build of `@causlts/core` — the
...
`tieredPropertyTrials` (published from `@causlts/core/testing`) or
...
`@causlts/core` itself stays on the deterministic TS engine. The
WebAssembly substrate landed by EPIC #680 (Phase-0 + Phase-1 scaffolding,
17 sub-issues merged) is reached through the opt-in `@causlts/core/wasm`
subpath:
