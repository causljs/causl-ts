...
> **Status.** Accepted. The Adoption Epic F PRs that originally referenced this document — #197 (migration guides), #198 (drift detector), #199 (validation procedure) — have all merged, and #225 added the runnable end-to-end harness. The catalogue ships at schema version `0.1`. The drift detector (`@causlts/migration-check`) consumes this document; the migration guides (`docs/migration/from-{jotai,mobx,redux}.md`) reference it by rule ID; the validation procedure (`docs/migration/validation.md`) cross-references it when reporting findings.
>
...
since: <semver of @causlts/migration-check that introduced the rule>
example_before: |
...
| `R-04` | important | `createAsyncThunk` → `@causlts/sync` `resource(graph, key, loader)` | A `createAsyncThunk` import or call. |
...
- **Bumping a rule's severity.** A breaking change to consumers' CI exit codes. Requires a major version bump on `@causlts/migration-check` and an entry in the changelog naming the rule and the rationale.
