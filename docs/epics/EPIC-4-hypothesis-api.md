...
```ts
import { hypothesis, afterCommit } from '@causljs/hypothesis'
...
```
packages/hypothesis/
├── package.json              # name: "@causljs/hypothesis", private: false, sideEffects: false
...
|  5 | Diamond glitches | `afterCommit({ any: true }, (s, app) => glitchFreedomInvariant(s, app))` | covered by `@causljs/glitch-freedom` adopter property test, not in this EPIC's acceptance |
...
The acceptance test in §spec-16.5-acceptance.test.ts covers rows 1, 6, 7, 8 directly. Row 5 (diamond glitches) is covered by the `@causljs/glitch-freedom` adopter property test, which uses this package's `afterCommit` combinator but is not in this EPIC's acceptance fixture set. Row 11 (use-after-dispose) is covered by both the static pass (EPIC-2) and a runtime hypothesis fixture; the runtime fixture lands as part of the acceptance test.
