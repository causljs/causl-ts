...
> `--hypothesis-out` flag and the `@causlts/hypothesis`
...
emitter and the `@causlts/hypothesis` evaluator's reader.
...
`@causlts/hypothesis` reader (`evaluateFromFile()` helper) remain
