...
import { resource, type ResourceHandle } from '@causlts/sync'
...
3. **`package_is_workspace_only_and_excluded_from_publish`.** Reads `package.json` and asserts `private === true`. Asserts no `publishConfig.access === 'public'`. Asserts the package name does not appear in any other package's `dependencies` (it appears only in `devDependencies` of `@causlts/sync` and the adapter's property test files). The grep is over the workspace's `package.json` files.
4. **`importing_at_causl_sync_does_not_pull_in_testing_internal`.** Spawns a tiny ESM script that does `import * as sync from '@causlts/sync'` and asserts `Object.keys(require.cache)` (or its ESM equivalent — `import.meta.resolve` + a Node `--experimental-vm-modules` introspection) does NOT contain a path under `packages/sync-testing-internal/`. The test is the structural witness that the production bundle of `@causlts/sync` cannot accidentally pull in the test infrastructure. The same shape is used today for `@causl/core-testing-internal`; we copy the assertion.
...
3. **Bundle isolation.** `@causlts/sync`'s production bundle does not include `@causl/sync-testing-internal`. The `peerDependencies` declaration on the testing-internal package (which ties it to `@causlts/sync` at workspace-link time) does NOT pull the testing-internal package's source into `@causlts/sync`'s production bundle, because `@causlts/sync` does not declare a `dependency` on the testing-internal package — only the eight property test files do, via the workspace-level `devDependency` we add to `packages/sync/package.json`. Test 4 is the structural witness.
...
3. The four-arm DU is closed by construction in `@causlts/sync`'s source; the property is the runtime witness against fuzzer-discovered escape.
...
2. **The `@causl/sync-testing-internal` package builds clean.** `pnpm --filter @causl/sync-testing-internal exec tsc --noEmit` exits 0; `pnpm --filter @causlts/sync test` discovers and runs all eight property files.
...
6. **The eight property suites are seed-deterministic.** `CAUSL_FUZZ_SEED=42 pnpm --filter @causlts/sync test` produces the same trial sequence on two consecutive runs; a counterexample on one workstation reproduces on every workstation.
...
8. **The package is bundle-isolated.** TASK 9.1 test 4 passes; importing from `@causlts/sync` does not transitively pull in `@causl/sync-testing-internal`; the production bundle of `@causlts/sync` is unchanged in size and shape.
