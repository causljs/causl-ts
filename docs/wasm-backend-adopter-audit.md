...
everything `createCausl(...)` returns, the `@causljs/react` adapter API,
and the documented adopter hook at `@causljs/core/internal` — and lists,
...
- **Adopter-visible?** — `Y` if the symbol is part of the documented
  ...
  only via the deep-import contract `@causljs/core/internal`. The N
...
| `NodeDisposedError`                                                                                             | class             | error       | Thrown when accessing a disposed node.                                                                                                                                   | Tombstone map lives WASM-side; `dispose` (the `@causljs/core/internal` adopter hook) writes the tombstone.                                                                                                                                                                                                                                                                                                                                                                             | Y                |
...
| `useCauslFamily(factory)` / `FamilyFactory` / `FamilyGraph`                                                     | hook / types      | react       | Per-key node identity within a provider; refcount-driven `dispose` via `@causljs/core/internal`.                                                                           | Disposal hits WASM-side (see `dispose` row). The refcount lives JS-side.                                                                                                                                                                                                                                                                                                                                                                                                             | Y                |
...
| `assertNever` (re-exported)                                                                                     | function          | (utility)   | Exhaustiveness probe surfaced from `@causljs/react`.                                                                                                                       | (no boundary).                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Y                |
| `VERSION` (`@causljs/react`)                                                                                      | value             | construct   | Pinned `'0.0.0'`.                                                                                                                                                        | (no boundary).                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Y                |
...
- Internal (N) rows: 8 — three core internals (`DisposalDuringCommitError`,
  ...
  `@causljs/core/internal` deep-import hooks.
...
interface BackendEngine {
  // Construct / lifecycle
  dispose(node: NodeId): void // from @causljs/core/internal
...
- **`@causljs/core/internal` capability narrowing
