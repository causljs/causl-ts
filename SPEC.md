...
> **Status (v0.9.0) — pre-deprecation marker (#1158).** The 3.0×-8.0× band below reflects the current TS engine's residual gap to MobX. The post-Rust-port band (epic #1133) is projected to narrow to **1.0×-4.0×** — adopter gates should reference engine version, not just the band ceiling, because a CI gate that anchors on the loose `× 8.0` upper bound will fire spuriously once the Rust port lands and the band re-tightens. See epic #1133 for the Rust port GO/NO-GO criteria. The 3.0×-8.0× band text below is the live SPEC requirement and is unchanged by this callout; the marker exists only to give adopters lead time before the band re-tightens.
...
**Current state (as of v0.9.0) — serde bridge raw-byte ceiling is currently violated (#1150).** The Tier 3 (`wasm-pkg/serde/`) artefact today ships at **213 KB raw / 66 KB Brotli**. Brotli is comfortably under the 80 KB ceiling; raw is over the 200 KB ceiling by 13 KB. The 200 KB raw / 80 KB Brotli row above remains the **target** ceiling — the SPEC does not relax the contract. The size-limit cell in root `package.json` (`@causl/core wasm bridge — serde-json (raw)`) was ratcheted to 230 KB by PR #1112 so the bundle-size CI gate absorbs the current 213 KB measurement; the gate therefore passes against the relaxed cell while the SPEC §17.6 row continues to document 200/80 as the target. This is recorded as a documented divergence per Option C of #1150, not a silent relaxation. **Resolution path:** the gap closes when the Rust engine port (epic #1133) lands and `wasm-opt` is invoked directly with the feature flags `wasm-pack` rejects today (per PR #1112's design discussion, tracked by #1085). At that point the size-limit cell tightens back to ≤200 KB raw and the §17.6 current-state paragraph is removed; the row's 200/80 contract has held in the SPEC throughout. Adopters who write CI gates directly against the §17.6 row (rather than against the size-limit cell) should expect the row's 200/80 contract to be the long-run target; until #1150 closes, the live artefact may exceed the raw cap by up to ~30 KB while remaining under the Brotli cap.
