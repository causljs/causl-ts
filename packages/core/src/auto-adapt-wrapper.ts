 ...
 *      backend (`@causlts/core/wasm`'s {@link loadWasmBackend}) can be
 ...
 * `subscribe`).
 */
const UNSEEN: unique symbol = Symbol('@causlts/core/auto-adapt-wrapper/unseen')
...
export function createAutoAdaptGraph(
  ...
  // Typed `unknown` so the wrapper does not pull `@causlts/core/wasm`
  ...
  function triggerMigration(): void {
    migrating = true
    // Dynamic import keeps `@causlts/core/wasm` out of the main bundle.
    ...
    void (async () => {
      try {
        ...
        // loader name confined to the lazily-imported `@causlts/core/wasm` chunk.
