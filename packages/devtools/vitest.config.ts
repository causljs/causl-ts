 ...
 *
 * Vitest configuration for the `@causlts/devtools` package. Aliases the
 * `@causlts/core` import specifier directly to the sibling package's
...
/**
 * Absolute path to `@causlts/core`'s TypeScript entry, resolved relative to
 * this config file so the alias works regardless of the caller's CWD.
 */
const coreSrc = fileURLToPath(new URL('../core/src/index.ts', import.meta.url))

/**
 * Absolute path to `@causlts/core`'s `/internal` entry — the adapter-level
 ...
 *
 * - `resolve.alias` redirects `@causlts/core` to its in-repo source so test
...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // is matched before the `@causlts/core` prefix is consumed.
      '@causlts/core/internal': coreInternalSrc,
      '@causlts/core': coreSrc,
    },
  },
  test: {
