 ...
 * its `set` method) from a public `@causlts/sync` export, the staged
...
import type { Tx } from '@causlts/core'
import {
...
// ─── Lock 3: Tx is NOT exported from @causlts/sync ────────────────────

// The @causlts/sync barrel must NOT re-export Tx. Adopters who need
// Tx must import from @causlts/core directly — that import path is
// the seam where the §17 commitment 7 review applies.
//
// Encoded as a type-level membership check. If `Tx` ever becomes a
// key of the @causlts/sync barrel, this AssertEquals trips at compile
