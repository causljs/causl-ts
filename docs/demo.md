...
`pnpm demo:spreadsheet` runs that demo locally. It builds `@causlts/core`, `@causlts/devtools`, and `@causlts/formula`, then serves a static page (`packages/formula/demo/index.html`) on `http://127.0.0.1:4173`.
...
| `replaceMany` | Re-exported from `@causlts/devtools`. |
