...
and loads it from `@causlts/core` through either an **N-API in-process**
...
- **Browser bite.** SPEC §17.6 commitment 14 "no host stranded" is
  **non-migratable** per CONSTRAINTS §2. Option (a) either ships a
  ...
  collapses the `@causlts/react` value proposition). Recommended:
...
| `@causlts/core` adopter API surface | `packages/core/src/index.ts`, `types.ts` | (no change) | The §1a non-negotiable surface (`createCausl`, `graph.input` / `derived` / `commit` / `read` / `subscribe` / `explain` / `simulate` / `snapshot` / `hydrate`, the React hooks, the error catalogue) is unchanged. Option (a) plugs in beneath the `BackendEngine` interface. |
...
| `@causlts/core/wasm` substrate distribution | `packages/core/wasm-pkg/` | (build artefact) | Same as bridges. |
...
resolver in `@causlts/core`'s native loader does
...
Adopter does `npm i @causlts/core`. npm walks
...
platform package resolves, `@causlts/core`'s install script
(`postinstall`) attempts `napi build` from a vendored source tree
shipped inside `@causlts/core`. If that fails (no `rustup`, no
network), `@causlts/core` falls through to the WASM/JS path (Path A1)
...
| Existing `release.yml` (`@causlts/core`, `@causlts/react`, etc.) | unchanged | unchanged |
...
`@causlts/core/native/index.ts` (new) does:
...
Browser path: `@causlts/core/native/index.ts` is gated by a
...
> The opt-in `@causlts/core/wasm` substrate ships against a documented
...
> the entire `@causlts/react` value proposition.
...
deleted. `@causlts/react` becomes Node-only (SSR-only); browser users
have no engine.
...
- Deprecation cycle: at least one minor version of warning on
  `@causlts/react`'s browser entry, codemod for adopters to migrate to
  a server-only architecture (Hydrate-only on the browser, all
  commits server-side).

**What this strikes**:

- The `@causlts/react` value proposition per CONSTRAINTS §2 framing.
...
`@causlts/sync`, bench harness, property-test host). The WASM stack
...
| **E. `@causlts/core` native loader** | `packages/core/src/native/index.ts`; `detectBackend()` extension for native tier; fallback chain wiring (native → wasm tier 1 → tier 2 → tier 3 → JS). | 1 | The fallback chain pattern is established; this is essentially a tier-prepend. |
...
Existing adopters today consume `@causlts/core` against either:
