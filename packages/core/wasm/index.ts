 ...
 *
 * `@causlts/core/wasm` — opt-in WASM backend entry point.
 *
 * This subpath is the lazy-load seam for the WebAssembly-backed
 * engine described in EPIC #680. Importing `@causlts/core` does **not**
 * pull this module in; only callers who explicitly
 * `import('@causlts/core/wasm')` (or who flip `createCausl({ backend:
 * 'wasm' | 'auto' })` past the auto-adapt threshold once #686 lands)
 * pay the WASM bundle cost.
 *
 * The cost shape:
 *
 *   - `@causlts/core` main bundle: tiny loader stub (~1 KB).
 *   - `@causlts/core/wasm`: ~60–120 KB compressed WASM module + ~5 KB
 ...
 * declarations) so the `@causlts/core/wasm` entry point resolves to the
 ...
 * elides `export type` statements at emit time — so the `@causlts/core/wasm`
...
export interface WasmBackendOptions {
   ...
   * `https://cdn.jsdelivr.net/npm/@causlts/core@<ver>/wasm/`) when
   ...
   *
   * Defaults to `'@causlts/core/wasm:<bridge>'` so unrelated callers
 ...
 * @example
 * ```ts
 * import { loadWasmBackend } from '@causlts/core/wasm'
 *
...
export class WasmBackendUnavailableError extends Error {
  ...
  constructor(bridge: BridgeId) {
    super(
      `@causlts/core/wasm: backend artifact for bridge '${bridge}' is not yet built. ` +
...
export class RustSsotDowngradedError extends Error {
  ...
  constructor(divergenceDetail: string) {
    super(
      `@causlts/core/wasm: engine: 'rust-ssot' graph sticky-downgraded to ` +
 ...
 * global (option-c doc §2.3) — multi-graph adopters (`@causlts/sync`,
...
class WasmBackend implements BackendEngine {
   ...
   * `@causlts/core/internal`'s `_migrateFrom(graph, snap)` so the
