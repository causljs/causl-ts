 ...
 * extension point.
 *
 * ## Why this lives in `@causlts/core`
 *
 * The canonical reducers — `reduceConflict` and `reduceResource` —
 * live in `@causlts/sync/src/statechart-reducers.ts` (PR #1056). The
 * BackendEngine seam in `@causlts/core` cannot import from `@causlts/sync`
 ...
 * determinism property test (mirrored on the `@causlts/sync` test side
 ...
 * `@causlts/core`, sync re-exports), but it churns the sync package's
 ...
 * reducers' control flow. Adopter callers on the `@causlts/sync` side
...
// ---------------------------------------------------------------------------
// ConflictRegistry reducer — mirrors @causlts/sync/src/statechart-reducers.ts
// ---------------------------------------------------------------------------

/**
 * Structural ConflictEvent shape. Mirrors the TS `ConflictEvent` DU
 * from `@causlts/sync/src/statechart-reducers.ts` exactly, repeated here
 ...
 * `reduceConflict` from `@causlts/sync/src/statechart-reducers.ts`.
 *
...
// ---------------------------------------------------------------------------
// ResourceFleet reducer — mirrors @causlts/sync/src/statechart-reducers.ts
// ---------------------------------------------------------------------------

/**
 * Structural ResourceEvent shape. Mirrors `ResourceEvent` from
 * `@causlts/sync/src/statechart-reducers.ts`. The promise slot is
 ...
 * `@causlts/sync/src/statechart-reducers.ts`. Generic `T` is structural
 * `unknown` at the seam.
 */
 ...
 * from `@causlts/sync/src/statechart-reducers.ts` exactly.
 *
 ...
 * @see `@causlts/sync/src/statechart-reducers.ts` — canonical contract.
 */
