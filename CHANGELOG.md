...
- `"sideEffects": false` on `@causlts/core`, `@causlts/sync`,
  `@causlts/react`, `@causlts/formula` package.json files. Enables
...
| --- | ---: | ---: | ---: |
| `@causlts/core` | **14.36 KiB** | 47.50 KiB | 76 KiB |
| `@causlts/sync` | 2.40 KiB | 2.38 KiB | 9 KiB |
| `@causlts/react` | 1.75 KiB | 12.73 KiB | 20 KiB |
| `@causlts/formula` | 2.96 KiB | 9.22 KiB | 16 KiB |
| **TOTAL** | **21.46 KiB** | 71.83 KiB | 121 KiB |

### Excluded from v0.2.0

- All WASM artefacts (`@causlts/core/wasm` subpath; the `gc-builtins`,
...
- `@causlts/devtools`, `@causlts/devtools-bridge`, `@causlts/hypothesis`,
  `@causlts/migration-check`, `@causlts/persistence`,
  `@causl/sync-testing-internal`.
- The `./internal` and `./testing` subpath exports on `@causlts/core`;
  the `./resource` and `./conflict` exports on `@causlts/sync`.
...
- **`@causlts/core/wasm` Phase-1 is a TS wrapper, NOT a Rust engine
...
- **React playground (`causl-org/playground/`) (#666).** Ports the
  ...
  HTML page under `causl-org/`. Loads Monaco `0.52.2` and `@causlts/core`
...
- **React spreadsheet demo (`causl-org/spreadsheet/`) (#666).** Ports
  ...
  `@causlts/core`, `@causlts/formula`, and `@causlts/devtools` from esm.sh.
...
- **CauslModel IR — SPEC §16.2.1.1/2 documented** (#569). The
  ...
  (`@causlts/core`) trips at PR time when SPEC text drifts from
  source.
- **`@causlts/sync`: `whyUpdated` / `whyNotUpdated` decoders +
...
- **`@causlts/hypothesis`: SPEC §16.5.1 surface expanded** (#571).
...
### Added
- `@causlts/core`: `graph.simulate(intent, run): SimulateResult` — the
