 ...
 *
 * Public type vocabulary for `@causlts/core`. Defines the branded
...
const _NODE_BRAND_TAG: unique symbol = Symbol('@causlts/core/Node')
/**
...
export interface CreateCauslOptions {
   ...
   * async `import('@causlts/core/wasm')` + `loadWasmBackend()` call.
   ...
   * but the canonical `@causlts/react` adapter holds the `read()`
   ...
   *     `@causlts/react`'s canonical hooks) suppresses H1 tracking for
   ...
   * `@causlts/core/wasm` subpath (which the main barrel must never pull
   ...
   * so `./types.js` does not depend on the `@causlts/core/wasm`
...
export interface Graph {
   ...
   *   (`@causlts/core/internal`). Disposal records a tombstone keyed
   ...
   * @throws {@link NodeDisposedError} if `node.id` has been released
   *   through the adapter-layer `dispose` hook
   *   (`@causlts/core/internal`). The typed disposal error mirrors
   ...
   * @throws {@link NodeDisposedError} if `node.id` has been released
   *   through the adapter-layer `dispose` hook
   *   (`@causlts/core/internal`). Adapter code branches on this typed
