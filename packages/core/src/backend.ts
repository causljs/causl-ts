 ...
 * `@causlts/sync/src/statechart-reducers.ts` exactly so the seam's
...
export interface BackendEngine {
   ...
   * exposed through `@causlts/core/internal`. Lives on the BackendEngine
   ...
   * #1068, wired by Sub-D of EPIC #680). Callers in `@causlts/sync`'s
   ...
   * in `@causlts/sync` and the BackendEngine seam in `@causlts/core`
   * cannot import them without inverting the package dependency
   * direction. Adopters on the `@causlts/sync` side hold typed values
