...
Opt-in WebAssembly backend entry point for `@causlts/core`.
...
| Surface                                     | Cost                                                                                                                                                                    |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@causlts/core` main bundle                   | Tiny loader stub (~1 KB). No WASM import.                                                                                                                               |
| `@causlts/core/wasm` (this)                   | Loader interface + bridge picker (~2 KB).                                                                                                                               |
...
const backend = await loadWasmBackend({
  wasmBaseUrl: 'https://cdn.jsdelivr.net/npm/@causlts/core@<version>/wasm/pkg/',
})
```
...
```ts
import { loadWasmBackend, detectBridge, WasmBackendUnavailableError } from '@causlts/core/wasm'
...
- #684 — JS bindings + lazy-load loader + `@causlts/core/wasm` entry. **Merged (PR #1031).**
