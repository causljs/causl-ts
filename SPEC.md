...
The Rust crates that ship today fall into two buckets, neither of which is the engine: `tools/checker/Cargo.toml` declares `serde`, `serde_json`, `clap`, `anyhow` for the static IR linter; and `tools/engine-rs-core/` plus `tools/engine-rs-bridge-{serde,gc}/` (per the WASM-backend EPIC #680, Phase-0 + Phase-1 scaffolding) carry the pure-algorithm state machine and the JS↔WASM bridges that back the opt-in `@causl/core/wasm` substrate. The engine itself — the value-of-record applications import as `@causl/core` — stays TypeScript.
