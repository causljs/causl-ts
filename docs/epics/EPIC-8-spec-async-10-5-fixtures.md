...
`@causlts/core`'s `createCausl`. If any of those exports drift, this EPIC
...
`pnpm --filter @causlts/sync test:run` on every PR. Readers reviewing
...
TypeScript that compiles `@causlts/sync`'s public API, run by the same
...
`createCausl()` from `@causlts/core`. If the public API's observable
...
void`) a maintenance liability vs importing `@causlts/react`'s
...
`@causlts/sync` alone, with no `@causlts/react` dependency. The dispatcher
...
commit lands at `now == 1`, not 0. If `@causlts/core` changes its
...
`@causlts/core` and the §5 single-pipeline guarantee says subscribers
...
exactly five `it()` blocks. Imports: `createCausl` from `@causlts/core`;
`resource` and the `ResourceState` type from `@causlts/sync`;
...
import from `@causlts/react`; the dispatcher is duplicated in-file
exactly so the test compiles against `@causlts/sync` alone.
...
1. **Equivalence pinning.** `expect(observedDirect).toEqual(
   ...
   `it()` block 1.
2. **MVU dispatcher self-contained.** No import from `@causlts/react`.
   The dispatcher is six lines, in-file, with a switch over the
   three message variants. We duplicate intentionally to keep
   `@causlts/sync`'s test suite buildable against `@causlts/sync` alone
   — `@causlts/react` has its own MVU tests on its own gate.
...
Imports: `createCausl` from `@causlts/core`; `Conflict` (type),
`createConflictRegistry`, `ForbiddenConflictTransitionError`,
`singleConflictWhen` from `@causlts/sync`; `describe`, `expect`, `it`
...
Imports: `createCausl` from `@causlts/core`;
`ForbiddenResourceTransitionError`, `resource` from `@causlts/sync`;
...
- `.github/workflows/ci.yml` — confirm the existing lane that runs
  ...
  `pnpm --filter @causlts/sync test:run` with the four files as
...
1. **Local dry-run: `pnpm --filter @causlts/sync test:run` discovers
...
2. **CI dry-run: open a PR with the four files and confirm the CI
   ...
   `@causlts/sync` package's `test:run` and reports the four files as
   passing.
...
- The four files are run by `pnpm --filter @causlts/sync test:run`.
...
- Bundle-budget sub-imports (EPIC-?). The four files import from
  `@causlts/sync`'s root, not sub-paths. If the bundle-budget EPIC
  introduces sub-path imports (`@causlts/sync/resource`,
  `@causlts/sync/conflict`), we revisit; today, root imports are the
...
- The `@causlts/react` MVU dispatcher's full surface. TASK 8.2 ships
...
- New public API on `@causlts/sync`. The four files consume the
  existing exports — `resource`, `ResourceHandle`, `ResourceState`,
