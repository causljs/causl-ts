...
- **TanStack Query** is the gold standard for server-state cache. It is not a general state engine; for client-side dependency graphs, you still need one of the others alongside it. Causl's `@causljs/sync` is *complementary*, not a replacement.
...
- **Not a spreadsheet engine.** `@causljs/formula` is a small package that demonstrates spreadsheet patterns on top of the core. It does not ship VLOOKUP.
...
| Path                          | Package                       | Role                                                                                  |
| ----------------------------- | ----------------------------- | ------------------------------------------------------------------------------------- |
| `packages/core/`              | `@causljs/core`              | Engine — Behaviors, derivations, transactions, snapshot/hydrate, retention, explain   |
| `packages/react/`             | `@causljs/react`             | React bindings — `useCausl`, `useDispatch`, `useCauslFamily`, MVU runner, SSR   |
| `packages/formula/`           | `@causljs/formula`           | Spreadsheet patterns *on top of* the core — formulas, ranges, cycles                  |
| `packages/sync/`              | `@causljs/sync`              | Async resources + conflict registry as composed statecharts                           |
| `packages/devtools/`          | `@causljs/devtools`          | Inspection primitives (explain materialisation, liveDerivation, snapshot, statechart) |
| `packages/devtools-bridge/`   | `@causljs/devtools-bridge`   | Redux DevTools Extension protocol bridge (zero-cost when absent)                      |
| `packages/persistence/`       | `@causljs/persistence`       | Persisted-input adapter with structured `PersistenceError` reporting                  |
| `packages/checker/`           | `@causljs/checker`           | npm wrapper for `causl-check` (Rust-backed static IR linter — twelve passes against the IR)               |
| `packages/bench/`             | `@causljs/bench`             | Benchmarks — Jotai / RTK / MobX comparisons across the canonical scenario taxonomy    |
| `packages/migration-check/`   | `@causljs/migration-check`   | Migration drift detector — flags unmigrated Jotai/MobX/Redux patterns in adopters     |

Internal-only `packages/core/testing/` (published as `@causljs/core-testing-internal`) provides shared property-test seam helpers.
...
The demos ship as static HTML pages under [`causl-org/`](./causl-org) — no build step, no framework install. Both load `@causljs/core` at runtime from esm.sh so they exercise exactly what an adopter installs.

- **[`causl-org/playground/`](./causl-org/playground/index.html)** (`https://causl.org/playground`) — the Quickstart example above in a Monaco editor wired to a live `@causljs/core` graph. Edit `derived`, watch the value update.
